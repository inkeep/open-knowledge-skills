# Codebase Wiki — Generate + Refresh

The user wants a **wiki of this codebase**, living in their own repo. Read the source code and author a navigable, diagram-rich, source-grounded wiki as markdown INTO the OK knowledge base, under `wiki/`. It is version-controlled, private by default, co-editable, renders in OK's live preview, and becomes grounding context for future agent sessions.

The content directory is the resolved `content.dir` — read it with `config({ key: 'content.dir' })` if you don't already know it. The wiki lives at `<content-dir>/wiki/`.
...
**Server requirement.** Phases that author or audit the wiki (`write`/`edit`/`links`/`search`) need the OK Hocuspocus server. Source reading (native tools) and git do not. If a `write` returns "Hocuspocus server is not running", tell the user to run `ok start` and retry — do NOT fall back to native file writes for the wiki markdown.

**Prerequisite.** This procedure assumes the `codebase-wiki` pack is seeded (`ok seed --pack codebase-wiki` → `wiki/` with `architecture/ modules/ flows/ concepts/ guides/`, each carrying folder frontmatter + a page template, plus `wiki/OVERVIEW.md` + `wiki/log.md`). If `exec("ls -A <content-dir>/wiki")` shows the layout is missing, tell the user to seed first, then start over.
...
| Knob | Values | Effect |
|---|---|---|
| `audience` | `internal` (default) or `public` | `internal`: dense; may cite internal infra; reference source via **relative links + symbol code-spans**. `public`: polished prose; **omit secrets, internal infra, ticket/PR numbers**; reference source via **GitHub blob URLs** (only if a remote exists — else fall back to relative). |
| `depth` | `tour`, `standard` (default), or `exhaustive` | `tour`: OVERVIEW + architecture + top flows only (fold modules into architecture). `standard`: + one page per package/module + concepts. `exhaustive`: + sub-module pages, `guides/`, per-flow failure modes, denser diagrams. |
...
- **Code references, `public`** → GitHub blob URLs (`https://github.com/<org>/<repo>/blob/<branch>/path/to/file.ts`) so a reader without the repo can follow them. Detect the remote with `git remote get-url origin`; if there is none, fall back to relative links.
...
`exec("cat <content-dir>/wiki/OVERVIEW.md")` and read its frontmatter:
...
## GENERATE — phased, STOP-gated

Work the phases in order. Do not skip or batch ahead of a STOP gate. Each page is authored with OK `write`/`edit`; create from the seeded templates (`write({ document: { path, template: "<name>" } })`) so pages start with the right skeleton, then fill the sections.
...
4. **Stamp frontmatter**: set `profile: <audience>/<depth>` and `source_commit: <git rev-parse HEAD>`. `source_commit` is the freshness anchor refresh mode diffs against — get it right.
...
## REFRESH mode (after code changes)
...
2. `git diff --stat <source_commit>..HEAD`. Inspect the changed paths.
...
- **Pack not seeded** — `wiki/` layout missing → tell the user to run `ok seed --pack codebase-wiki`, then start over. Exit.
- **Server down** — a `write`/`links` call reports the server is not running → tell the user to run `ok start` and retry. Exit cleanly; resuming keeps already-written pages.
- **User aborts at a STOP gate** → exit cleanly, leaving any already-written pages in place.
